...
def margin_loss(logits: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    """
    returns: true - other, shape (B,)

    > 0 : correct class still higher than every wrong class
    < 0 : some wrong class has exceeded the true class (attack success)
    """
...
# =========================================================
# Fast attacker-side forward with EOT (for evaluation logits)
# =========================================================
@torch.no_grad()
...
# =========================================================
# Fast attacker-side EOT margin
# =========================================================
@torch.no_grad()
def defended_margin_for_attacker(
    model: CLIPZeroShot,
    x: torch.Tensor,
    y: torch.Tensor,
    use_multiview: bool,
    ...
    if not use_multiview:
        ...
        for _ in range(M):
            ...
            m = margin_loss(shaped_logits, y)
            probs_before = base_logits.softmax(dim=-1)
            probs_after = shaped_logits.softmax(dim=-1)
            score_shift = (probs_after - probs_before).abs().sum(dim=1)

            margin_sum = m if margin_sum is None else (margin_sum + m)
            score_shift_sum = score_shift if score_shift_sum is None else (score_shift_sum + score_shift)
            cross_view_std_sum = base_cross_view_std if cross_view_std_sum is None else (cross_view_std_sum + base_cross_view_std)
        ...
        return margin_sum / float(M), out_aux
...
def square_attack_with_logging(
    ...
    best, aux0 = defended_margin_for_attacker(
        model=model,
        x=x_adv,
        y=y,
    ...
    initial_best = best.detach().clone()
    ...
    for i in range(attack_cfg.n_iters):
        ...
        loss_new, aux_new = defended_margin_for_attacker(
            model=model,
            x=x_new,
            y=y,
            use_multiview=defense_cfg.use_multiview,
        ...
        margin_history.append(best.detach().clone())
    ...
    log = {
        ...
        "initial_margin_mean": initial_best.mean().item(),
...
def eval_defense_family(
    ...
    print(f"Attack objective: EOT-averaged margin loss")
    ...
    for dcfg in defense_list:
        ...
        print(f"Initial margin mean:       {val['initial_margin_mean_sum'] / nb:.6f}")
...
#     # margin_loss 会用 y 去索引 logits 里“正确类别”的 logit，再和“错误类别里最大的 logit”做差。
#     # best 一般表示每个样本当前的 margin，所以 shape 通常是：[B]
#     # 如果 best[i] > 0，说明正确类别仍然最大，还没攻击成功
#     # 如果 best[i] < 0，
# # 说明某个错误类别分数已经超过正确类别，样本被攻击成功
#     accepted_steps = 0
#     total_steps = attack_cfg.n_iters * B
#     improvement_sum = 0.0
#     score_shift_sum = aux0["score_shift_l1"].sum().item()
